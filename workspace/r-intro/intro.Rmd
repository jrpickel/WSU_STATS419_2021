...
\newpage
# Letter count
The function ``letter_count(string)`` will take a string and will count the number of occurrences for each letter in the string. The ``string`` parameter for the function ``letter_count`` can be any kind of string data type.  
...
```{r,warning=FALSE}
fileName <- draft_file; # declare the file as a variable
...
```{r,warning=FALSE}
fileName <- final_file # declare the file as a variable
...
This is a bar plot to help visualize the count of each letter.\par
```{r,Final,fig.cap="Letter count for the Declaration of Independance signed by Congress"}
# generate frequency of letters
